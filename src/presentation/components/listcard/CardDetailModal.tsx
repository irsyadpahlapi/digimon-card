        ...
        <div className="p-6 space-y-6">
                ...
                {item.nextEvolutions.map((evolution) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={evolvingToId === evolution.id}
                      className={`w-full ${
                        evolvingToId === evolution.id
          ...
          <div className="flex gap-4 pt-4 border-t border-gray-200">
            ...
            {item.nextEvolutions &&
              ...
              (() => {
                const isAnyEvolving = evolvingToId !== null;
                const isDisabled = isAnyEvolving || item.total < 3;
                let buttonClass = '';
            ...
            {(() => {
              const hasNextEvolutions =
                Array.isArray(item.nextEvolutions) && item.nextEvolutions.length > 0;
              const canSell = item.isEvolution || !hasNextEvolutions;
              const disabled = isSelling || !canSell;
              const sellButtonClassesBase = hasNextEvolutions ? 'flex-1' : 'w-full';
              const sellButtonStateClasses = disabled
                ? 'bg-gray-300 cursor-not-allowed'
                : `${GRADIENT_WARNING_BUTTON} hover:shadow-xl transform hover:scale-105`;
              const sellButtonClasses = `${sellButtonClassesBase} ${sellButtonStateClasses} text-[#643c30] font-bold py-4 px-6 rounded-xl shadow-lg transition-all duration-200 flex items-center justify-center gap-2`;
              return (
                <button
                  onClick={() => {
                    if (!disabled) onSell?.(item.id, item.sellingDigimon);
                  }}
                  disabled={disabled}
                  title={
                    canSell
                      ? undefined
                      : 'Cannot sell: evolve this card first or wait until no evolutions remain.'
                  }
                  ...
                  {isSelling ? (
                    ...
                    <>
                      ...
                      <span className="text-sm">
                        {canSell ? `Sell (${item.sellingDigimon} coins)` : 'Cannot Sell'}
                      </span>
                    </>
                  )}
                </button>
